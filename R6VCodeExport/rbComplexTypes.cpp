...
void operator << ( upArchive& A, rbExportTable& D )
{
	...
	for(int i=0; i!=D.Items.size(); ++i )
	{
		...
		int count = D.Items[i].unk[9];
		if( count > 0 )
		{
			dword* data = new dword[count*2];
			A.Serialize(data,sizeof(dword)*count*2);
...
wxString rbExportTable::Item::Print()
{
	...
	s += wxString::Format(wxT("0x%X, "), unk[6]);
	s += wxString::Format(wxT("%d, "), unk[7]);
	s += wxString::Format(wxT("0x%X, "), unk[8]);
	s += wxString::Format(wxT("%d, "), unk[9]);
...
wxString rbExportTable::Item::PrintContext( rbPackage& p )
{
	...
	s += wxString::Format(wxT("%d=%s, "), unk[4], PF_STR(p.GetObjectName(unk[4])));
	s += wxString::Format(wxT("%d, "), unk[5] );
	s += wxString::Format(wxT("0x%X, "), unk[6]);
	s += wxString::Format(wxT("%d, "), unk[7]);
	s += wxString::Format(wxT("0x%X, "), unk[8]);
...
{
	return wxString::Format(wxT("%d, %d, %d, %d"), ClassPackage, ClassName, Package, ObjectName);
}
...
*/
	...
	for(int e=0; e!=ExportTable.Items.size(); ++e )
	{
		...
		A.Seek( ei.GetSerialOffset() );
		UP_OFFSET_GUARD(rbsFunction,A,A.Tell(),A.Tell()+ei.GetSerialSize());

		/*wxLogMessage( wxString::Format(wxT("Func[%i]=(%s) 0x%.8x 0x%.8x")
			, e
...
void rbPackage::DumpByteCode( upArchive& A )
{
	...
	for(int e=0; e!=ExportTable.Items.size(); ++e )
	{
		...
		wxLogMessage( wxString::Format(wxT("DumpByteCode: [%u] [%s] [%s][%s]")
			, ei.GetSerialSize(), PF_SAFESTR(sdebug), PF_SAFESTR(GetNameString(ei.unk[3])), PF_SAFESTR(GetObjectName(ei.unk[2]))  ) );
		...
		rbBytecode bc(*this,A,func.scriptsize,e+1);
		bc.Parse();
